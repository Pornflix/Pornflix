...
Search.prototype.draw = function(json) {
	...
	if(json.results) {
		...
		for(var i = 0; i < json.videos.length; i++) {
			...
			previewLink[i].setAttribute("href", "/?view=video&id=" + json.videos[i].id);
			preview[i] = Helper.safeElement("img", "preview", previewLink[i]);
			preview[i].setAttribute("src", "../" + Constants.dataDir + "/" + json.videos[i].id + "/preview.jpg");
			...
			Helper.safeTextNode(json.videos[i].name, videoTitle[i]);
		}
	} else {
		videoContainer.style.textAlign = "center";
		Helper.safeTextNode("No results found", videoContainer);
	}
}
